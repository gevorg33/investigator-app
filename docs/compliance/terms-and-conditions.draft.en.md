# Terms & Conditions — DRAFT FOR LEGAL REVIEW

> **Not legal advice. Not for publication.** A structured draft prepared by engineering so
> counsel can work from the actual product. Bracketed items are unresolved decisions — see
> `counsel-brief.md`. Every limitation and exclusion below requires counsel's assessment
> against the consumer-protection regimes in the launch jurisdictions.

Covers the commercial terms of transactions between users. Platform–user matters are in the
Terms of Service.

---

## 1. Our role

The platform connects customers seeking investigation services with independent
investigators offering them. It provides the tools through which they agree scope, transact,
communicate and exchange deliverables.

Investigators are independent professionals. They are not employees, agents, partners or
representatives of the platform. The platform does not perform investigations, does not
direct how work is carried out, and does not control an investigator's methods.

[**Counsel: does the platform become a party to the customer–investigator contract, or does
it facilitate a contract between them? This determines the customer's counterparty and much
of what follows. Note the platform holds funds, adjudicates disputes and can order refunds,
which cuts against pure facilitation.**]

## 2. What verification means, and what it does not

Verification confirms that documents supplied by an investigator were checked against their
stated identity, professional standing and declared scope of practice at the time of review.

Verification is **not** a warranty of competence, quality, outcome, or of continued
compliance. It does not transfer an investigator's professional or legal responsibility to
the platform.

[**Counsel: this is the platform's principal exposure. Displaying "verified" is a
representation. Advise on wording and on what continuing monitoring obligation, if any,
attaches.**]

## 3. Customer responsibilities and obligations

A customer must:

- Request only work that is lawful where it will be carried out
- Confirm, before submitting a mission, that their purpose is lawful
- Describe the work accurately and completely, including anything affecting lawfulness
- Hold any legal basis they assert, and provide it on request
- Provide only information and materials they are lawfully entitled to provide
- Not use the platform to harass, stalk, intimidate or unlawfully monitor any person
- Handle received evidence and reports lawfully, including data-protection obligations owed
  to the people they concern
- Conduct all engagement and payment through the platform
- Not misrepresent their identity or their relationship to the subject of a mission

A customer is responsible for their use of what is delivered. Obtaining information lawfully
does not make every subsequent use of it lawful.

### Materials submitted with a mission

Customers must ensure that all information, documents, images, recordings and other materials
they include in a mission are lawful to collect, share and process.

A mission may contain only:

- Information that is publicly accessible and lawfully obtained
- Information the customer has the legal right, or explicit authorisation, to share
- Information necessary and relevant to the requested investigation
- Data that does not unlawfully expose another person's private, confidential or sensitive
  information

A customer must not include, or request work relying on:

- Private or confidential information obtained without authorisation
- Personal data whose sharing would breach applicable privacy or data-protection law
- Sensitive information unrelated to the investigation
- Stolen, hacked, unlawfully accessed or otherwise illegally obtained material
- Information intended to harass, threaten, intimidate, discriminate against or harm any person
- Requests to access private accounts, devices, communications, locations or records without
  lawful authorisation

Customers are responsible for ensuring that their missions comply with applicable privacy,
data-protection and other relevant law. The platform, investigators and staff must respect the
privacy, dignity, rights and lawful interests of every individual involved, including people
who are not users of the platform.

[**Counsel: a customer warranty allocates risk between the parties but does not by itself
discharge the platform's own obligations as a controller of the data it stores, nor the
investigator's as a processor of it. Advise on how far this clause can be relied upon, and what
the platform must do independently of it. Relates to counsel-brief.md §§3, 8–11.**]

### Where a mission contains problematic material

Where a mission contains unlawful, unauthorised or excessively sensitive material, the platform
or the assigned investigator may refuse, suspend, modify or cancel it, and may require the
customer to remove or replace the material before the work proceeds.

A distinction applies to the remedy:

- Material that is **lawful but excessive or irrelevant** — the customer is asked to reduce or
  remove it, and the mission may continue once they do
- Material that is **unlawful or unauthorised** — the mission does not proceed until it is
  resolved, and the material is withdrawn from circulation

Material already incorporated into the case record is withdrawn from further use and marked,
rather than erased, so that the integrity of the record and any obligation to retain it are
preserved.

An investigator who refuses work on these grounds is acting properly, and doing so is not
treated as a failure to deliver.

[**Counsel: retention where material is unlawful but may also be evidence of a violation, or
subject to a legal hold. Deleting on request may conflict with an obligation to preserve.**]

## 4. Investigator responsibilities and obligations

An investigator must:

- Hold and maintain every licence, registration and permission required where they work
- Work only within their licensed and declared scope
- Comply with all applicable law, including surveillance, recording, data protection and
  trespass law
- Decline and report missions that would require unlawful activity
- Not accept instructions to act unlawfully, regardless of payment or customer insistence
- Deliver the scope stated in their accepted quote
- Handle evidence lawfully and proportionately, collecting no more personal data than the
  mission requires
- Keep declared specialties, service areas, languages and availability accurate
- Maintain the confidentiality of customer information and of evidence
- Not share account credentials or delegate an assignment
- Conduct all engagement and payment through the platform
- [**Counsel: professional indemnity insurance — required, recommended, or verified?**]

An investigator is solely responsible for the lawfulness of their methods. Neither a
customer's instruction nor the platform's acceptance of a mission constitutes authorisation
to act unlawfully.

## 5. Prohibited activities

Neither party may use the platform to request, offer, facilitate or carry out:

- Unauthorised access to accounts, devices, systems, networks or communications
- Installation or use of spyware, stalkerware or covert monitoring software
- Location tracking of a person without a lawful basis
- Surveillance, monitoring or testing of a spouse, partner or former partner on suspicion
  alone, however described — including fidelity checks and loyalty tests. Work arising from
  actual legal proceedings, where the customer has standing, is not covered by this exclusion
- Recording of persons or communications where unlawful
- Unlawful entry onto property
- Harassment, intimidation or stalking
- Obtaining financial, medical, telecommunications or other protected records without lawful
  authority
- Impersonation of a person, official or authority
- Obtaining information for the purpose of unlawful discrimination, violence or coercion
- Circumventing protective orders or similar legal restrictions
- Any activity unlawful where it is carried out

This list is not exhaustive. Work may be refused as unlawful whether or not it is named.

## 6. User liability

Each user is responsible for their own actions, requests, services and activities, including
compliance with all applicable law, and for the consequences of them.

Customers are responsible for the lawfulness of what they request and how they use what they
receive. Investigators are responsible for the lawfulness of how they work and what they
collect.

[**Counsel: indemnity from users to the platform — scope, and enforceability against
consumers in each launch jurisdiction.**]

## 7. Missions, quotes and assignments

- A mission is a request, not an offer, and creates no obligation on anyone.
- Missions are screened before publication and may be rejected or held for review.
- A quote is an offer by an investigator, binding on them until its stated expiry.
- A quote states scope, deliverables, assumptions, exclusions, price, estimated duration,
  expiry and cancellation terms. **What is not in the accepted quote was not agreed.**
- Acceptance plus successful payment authorisation creates an assignment on the terms of
  that quote.
- One mission produces one assignment. Changes of scope require a new quote.
- Where an accepted quote is ambiguous, it is construed against the investigator, who drafted
  it.

## 8. Payment, fees and refunds

- Customers pay on acceptance. Funds are held and released on completion.
- Platform fees and applicable taxes are disclosed before the customer confirms. No amount is
  charged that has not been shown.
- Refund entitlement is governed by the cancellation terms of the accepted quote, or by a
  dispute determination. [**Counsel: is there a platform-level refund floor, or is it
  entirely per quote? Consumer law in some jurisdictions will impose one regardless.**]
- Payment outside the platform is prohibited and carries no dispute, refund or chargeback
  protection.
- [**Counsel: chargeback allocation between platform and investigator.**]

## 9. Evidence, reports and intellectual property

- Evidence and reports are private to the assignment's participants.
- Evidence is immutable once submitted; corrections are new versions and both are retained.
- Exported material is watermarked with recipient and time.
- Retention is governed by the retention policy. Material is not kept indefinitely.
- [**Counsel: who owns the report and the evidence — investigator, customer, or licensed to
  the customer? Affects reuse, resale and what survives account deletion.**]

Both parties remain subject to data-protection law regarding the people the material
concerns, including people who are not users of the platform.

## 10. Disputes between users

- Disputes are raised through the platform and reviewed by staff.
- Review compares what the accepted quote stated against what was delivered.
- Outcomes may include a required revision, partial refund, or full refund.
- Decisions are recorded with their reasoning and communicated to both parties.
- [**Counsel: is the determination binding, and does it exclude or merely precede other
  recourse? Cannot exclude non-waivable consumer rights.**]

## 11. Reporting abuse, illegal activity and policy violations

Any user may report a mission, message, profile or assignment through the reporting controls
present on each. Reports are reviewed by staff and their outcomes recorded.

Users must report requests or conduct they believe unlawful rather than silently declining.

The platform may refer matters to regulators or law enforcement where it considers it
appropriate or is legally required to do so. [**Counsel: mandatory reporting obligations by
jurisdiction, and what these terms should commit us to.**]

## 12. Suspension and termination

Access may be suspended or terminated where a user:

- requests, offers or performs prohibited activity
- provides false identity, licensing or qualification information
- works outside their licensed scope
- solicits or accepts payment outside the platform
- shares account credentials or delegates an assignment
- mishandles evidence or breaches confidentiality
- abandons assignments or repeatedly fails to deliver agreed scope
- behaves abusively toward another user or staff
- breaches these terms or the Lawful Use Policy

Additionally, for customers, submitting materials that breach the requirements in section 3,
including unlawfully obtained material or personal data shared without authorisation. Depending
on severity this may result in removal of the material, cancellation of the mission,
restrictions on the account, suspension, or permanent termination.

Where the law requires or permits it, serious violations may be reported to the appropriate
authorities.

Additionally, for investigators, substantiated violation of professional or ethical
obligations, including:

- performing an investigation improperly or negligently
- deceiving or misleading a client
- accepting an assignment and abandoning it without valid reason
- delivering work that fails to meet the agreed scope
- disclosing confidential client, investigation or assignment information to a competitor or
  any unauthorised third party

A **permanent ban** may follow a violation that has been substantiated through the review
process below. It is reserved for conduct that makes continued participation untenable, and is
not the ordinary response to a single dispute resolved against an investigator.

### Review process

Before a permanent ban:

1. The investigator is notified of what is alleged and on what basis
2. They are given a reasonable opportunity to respond
3. The decision is made by a person not involved in the underlying complaint
4. The decision and its reasoning are recorded and communicated
5. An appeal to a different reviewer is available

An account may be suspended immediately as a precaution where leaving it active would risk
customers or their data. A precautionary suspension is not a ban, and the full process still
follows.

### Effect on money and live assignments

A ban is not a financial penalty. Amounts earned for work already delivered remain payable,
subject to any refund or determination owed to a customer, which is decided separately on its
own merits.

Live assignments are resolved — reassigned, completed or refunded — as part of the action, so
that customers are not left without recourse.

[**Counsel: notice and appeal periods; the evidentiary standard for a permanent ban; whether
set-off against amounts owed is enforceable in the launch jurisdictions; obligations on
termination regarding data and funds; and the lawful basis for retaining a ban record after
account deletion — see the Privacy Policy and counsel-brief.md.**]

## 13. Limitation of liability

[**Counsel: to be drafted. Engineering notes the factual position — the platform does not
perform investigations, does not direct methods, and does not control what an investigator
collects; but it does verify investigators, screen missions, hold funds, adjudicate disputes
and store evidence. Limitations must be assessed against consumer protections in each launch
jurisdiction, several of which will not permit exclusion of liability for our own acts.**]

## 14. Changes to these terms

Material changes require acceptance before continued use, with notice before the effective
date. Non-material changes are notified without re-acceptance. Whether a change is material
is determined by the platform's compliance owner.

Every acceptance records the document version, the exact text accepted, and the language in
which it was shown.

[**Counsel: notice period for material changes; which language version governs.**]
