 ...
 * investigators through registered tools over live data. The chat model is null until
 * `OPENAI_API_KEY` and `OPENAI_CHAT_MODEL` are set, and both endpoints answer 503 until then.
 */
@Module({
  ...
  imports: [AuthModule, KnowledgeModule, SearchModule, TaxonomyModule],
  controllers: [AssistantController, DiscoveryAnswerController],
  providers: [
