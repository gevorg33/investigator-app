 ...
 * shared one database; each worker now has its own, emptied before the next spec file. The
 * seeding transaction also ran ANALYZE on rows no other transaction could see, so an autoanalyze
 * firing in between would replace the statistics with an empty table's and the planner would
 * choose differently — a failure that appeared only under the load of a full suite.
 *
 * The query is the exported one — a lookalike rewritten for the test would prove the test's SQL
 * is fast, which is not the question.
 */
